...
def makePointProjectionPairs(inequalities, K, bound=1.0, nproj=100):
    """
    Generates pairs of points and their projections onto the intersection of linear inequalities

    Inputs:
        - inequalities : dictionary of linear inequalities (generated by makeRandomData)
        - K            : number of point-projected point pairs
        - bound        : scalar bound of points to generate (points will be picked uniformly from [-bound,bound]^d)
        - nproj        : number of times to project onto each hyperplane

    Output: dictionary containing
        - P            : 2 x K array of initial points
        - Pproj        : 2 x K array of initial points after projection onto intersection of linear inequalities
    """
    
    print('Bound for test data: {0:f}'.format(bound))
    d = inequalities['A'].shape[1]
    n = inequalities['b'].shape[0]
    P = bound*(np.random.random((d,K))*2.0 - 1.0)
    Pproj = np.zeros((d,K))
    for i in range(K):
        p = P[:,i]
        pproj = p
